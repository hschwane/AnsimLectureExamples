...
constexpr int wgsize = 32; // cuda work group size
...
// we can not return early, all threads must participate in filling the shared memory
// meaning for now this algorithm only works if N is a multiple of the workgroup size
// implementing the proper guards for arbitrary particle numbers is possible however,
// and left as an exercise for the reader
//    if(i > particleCount)
//        return;
    __shared__ float4 sharedPositions[wgsize];

    float3 posi = make_float3(pos[i]);
    float3 acci{0,0,0};

    for(int jstart=0; jstart<particleCount; jstart+=wgsize)
    {
        sharedPositions[threadIdx.x] = pos[jstart+threadIdx.x];
        __syncthreads();
        for(int j = 0; j < wgsize; j++)
        {
            float3 posj = make_float3(sharedPositions[j]);
                        ...
                        ParticleRenderer& renderer)
{
    ...
    if(ImGui::Begin("Simulation"))
    {
        ...
        if(ImGui::CollapsingHeader("Rendering Mode"))
        {
            ...
            {
                if(uiParticleCount % wgsize != 0) {
                    ImGui::SimpleModal("Invalid Particle Count", "Particle count must be a multiple of " +
                            std::to_string(wgsize), {"OK"} );
                    return;
                }
